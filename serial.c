...
// function to check if there is data to be read in the TX buffer
// 1: there is data in the buffer
// 0: nothing in the buffer
...
// circular buffer functions for VX
// retrieve a byte from the buffer
char getCharFromVxBuf(void){
    if (VxBufReadCnt>=VX_BUF_SIZE) {VxBufReadCnt=0;} 
    return EUSART4VXbuf[VxBufReadCnt++];
}

// add a byte to the buffer
void putCharToVxBuf(char byte){
    if (VxBufWriteCnt>=VX_BUF_SIZE) {VxBufWriteCnt=0;}
    EUSART4VXbuf[VxBufWriteCnt++]=byte;
}

// function to check if there is data in the VX buffer
// 1: there is data in the buffer
// 0: nothing in the buffer
char isDataInVxBuf (void){
    return (VxBufWriteCnt!=VxBufReadCnt);
}
